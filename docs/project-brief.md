...
5. **Correlation, deduplication & failure clustering** — multi-diagnostic/multi-tool failure reports collapse into real clusters (Section 1 shipped only trivial one-diagnostic-per-cluster). *(done — `parsing/clustering.py`: `dedup_key` deliberately excludes `parser`, because all four parsers call `find_process_failure` and co-firing parsers emit byte-identical `PROCESS_FAILURE`s that must collide. Rule ladder S1/C1/C2a/C2b/C3 under guard G1 — two different non-None `test_id`s never share a cluster, since the test is the unit of failure. Clustering is non-lossy: dedup is cluster membership + a `DiagnosticRole`, never a deletion from `report.diagnostics`. **No rule keys on `section_id`** — it is `null` for every diagnostic in every committed fixture, because GitHub Actions closes the `##[group]` before step output, so such a rule could never fire.)*
6. **Scale & robustness** — bounded memory and perf, consuming the `ParseLimits` seam Section 1 already built. *(done, with two decisions recorded rather than built. **Streaming is DECLINED (D1):** two of four parsers plus `find_process_failure` and the cluster builder all need the complete line list, so streaming means rewriting every parser — and a stream fed by line iteration cannot reproduce `str.splitlines()`'s boundary set, making it non-deterministic against `parse_log` on exactly the degenerate inputs it was meant to handle. What shipped instead is `_bounded_splitlines`, which stops at `max_total_lines` without materializing the rest, plus a `max_total_chars` valve, so peak memory tracks the limits rather than the input. **`SourceRange` byte offsets are DECLINED (D6)**, closing that deferral: they are not derivable from `splitlines()` output, and would be the only field in a coordinate system no other field shares. Baselines in `docs/perf-baselines.md`.)*
7. **Security & observability hardening** — the full secret-masking provider matrix and metrics. *(done — 13 ordered rules at the single `normalizer.py` choke point, `mask_counted` for per-class stats, and a `_base_stats()` key set that success and fatal paths share identically. **Entropy heuristics are rejected outright**, not deferred: on the anchor fixture a canonical entropy rule masks six action-pin SHAs, two UUIDs, every wheel hash and the cache key — the false-positive cost lands precisely on the evidence the engine exists to preserve. The top risk in this section is over-masking, and masking that destroys evidence is a bug.)*

   **Confidence calibration was listed under Section 7 above and was NOT built.** Section 7 as implemented covers security + observability only and leaves `confidence.py` untouched. It needs either its own increment or a documented drop, and it is better decided against Slice 6's labeled corpus than guessed now. Recorded here and in `CLAUDE.md` §2 rather than silently dropped.
...
*Done (whole slice) =* `raw log → FailureReport`, with deterministic tests and real fixtures for every supported tool/ecosystem. *Done (Section 1, met) =* the engine spine, provider seam, `ParseLimits`, and total-function guarantee are in place and validated against the anchor fixture with golden-snapshot + fuzz/security tests, so Section 2's pytest parser plugs in with no core change. *Done (Sections 2+3, met) =* the pytest parser is the anchor fixture's primary cluster (`tests/test_types.py::test_file_surrogates[type1]`, `report.exit_code == 1` preserved via a shared `process_failure.py` extraction so the specialized parser co-emits it instead of relying on GenericParser's now-suppressed fallback), and the same `StackTrace`/`TestOutcome` spine is proven on a second ecosystem (jest + vitest) through one shared `JsTestParser` — no core rewrite in either case. *Done (Section 4, met) =* `CompilerParser` emits structured `COMPILER_ERROR`/`LINT_ERROR` diagnostics for tsc and eslint, registered in `PARSER_REGISTRY` ahead of `GenericParser`, co-emitting the shared `PROCESS_FAILURE` — again no core rewrite. *Done (Sections 5-7, met) =* `report.clusters` stops mirroring `report.diagnostics` 1:1 and the anchor's two diagnostics collapse to one cluster carrying a `CONSEQUENCE`; peak memory is bounded by `ParseLimits` rather than by input size; and no known secret literal survives into `to_json` from any of five plant positions, while the anchor's `LogSection.title` and `Diagnostic.message` values stay byte-identical to their pre-Section-7 values. `SCHEMA_VERSION` walked 1.1 → 1.2 → 1.3, additively throughout.
